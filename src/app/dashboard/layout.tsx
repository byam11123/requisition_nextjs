...
import { LayoutDashboard, Wrench, CalendarCheck2, HandCoins, FilePlus, Users, User, LogOut, Loader2, ChevronLeft, LucideIcon, ReceiptText, Settings2, ShieldCheck, BriefcaseBusiness } from 'lucide-react';
      ...
      <aside className={`relative transition-all duration-300 bg-slate-900/50 border-r border-white/5 backdrop-blur-xl flex flex-col shrink-0 ${isSidebarOpen ? 'w-64' : 'w-22'}`}>
        ...
        <nav className={`flex-1 overflow-y-auto py-5 ${isSidebarOpen ? 'px-4' : 'px-3'}`}>
          ...
          <div className="space-y-2">
            ...
            {canAccessDashboardPath('/dashboard', user.role, user.pageAccess, user.rolePageAccess) && navItem('/dashboard', 'Requisition', ReceiptText)}
            {canAccessDashboardPath('/dashboard/repair-maintainance', user.role, user.pageAccess, user.rolePageAccess) && navItem('/dashboard/repair-maintainance', 'Repair & Maintainance', Wrench)}
              ...
              <div className="mb-4 rounded-2xl border border-white/5 bg-slate-950/30 p-3">
                <div className="flex items-center gap-3">
                  ...
                  <div className="min-w-0 overflow-hidden">
                    ...
                    <p className="truncate text-xs uppercase tracking-[0.2em] text-slate-500">{user.customRoleName || user.role}</p>
                  </div>
                </div>
              </div>
