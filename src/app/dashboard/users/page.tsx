...
import { Plus, Search, Edit, MoreVertical, CheckCircle, XCircle, Trash2, Loader2, X, ShieldCheck } from 'lucide-react';
...
const userRoleFilterOptions: FilterDropdownOption<string>[] = [
...
const userStatusFilterOptions: FilterDropdownOption<string>[] = [
...
const designationPickerBaseOption: FormSelectOption<string> = {
  value: '',
  label: 'Select saved designation',
};

const designationPickerCustomOption: FormSelectOption<string> = {
  value: '__custom__',
...
function CreateUserModal({ open, onClose, onSuccess, roles, designations }: any) {
  ...
  const designationOptions: FormSelectOption<string>[] = [
    designationPickerBaseOption,
    ...designations.map((designation: DesignationOption) => ({
      value: designation.key,
      label: `${designation.name}${designation.department ? ` (${designation.department})` : ''}`,
    })),
    designationPickerCustomOption,
  ];
      ...
      <div className="w-full max-w-md max-h-[90vh] overflow-y-auto rounded-3xl border border-white/10 bg-slate-900 p-5 shadow-2xl animate-fade-in-up sm:p-6 lg:p-8">
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div>
            ...
            <FormSelect
              value={form.customRoleKey}
              options={roleOptions}
              onChange={(value) => setForm((current) => ({ ...current, customRoleKey: value }))}
            />
          </div>
          <div>
