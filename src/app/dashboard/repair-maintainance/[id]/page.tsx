...
import { ArrowLeft, Loader2, X, CheckCircle, IndianRupee, Package, Phone, User, Flag, Truck } from "lucide-react";
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/70 backdrop-blur-sm">
      <div className="w-full max-w-lg rounded-3xl border border-white/10 bg-slate-900 p-5 sm:p-6 lg:p-8">
        <div className="mb-6 flex items-start justify-between gap-4"><h2 className="text-lg font-semibold text-slate-100 sm:text-xl">Update Payment</h2><button onClick={onClose} className="p-2 rounded-xl hover:bg-white/5 text-slate-400"><X size={20} /></button></div>
...
export default function RepairDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const timelineEvents: TimelineEvent[] = timelineSteps.map((step, i) => {
    ...
    const isCurrent = i === activeStep && !step.skipped;
    return {
      key: `${step.label}-${i}`,
      title: step.label,
    ...
    <div className="max-w-6xl mx-auto space-y-6 animate-fade-in-up">
      ...
      <div className="flex flex-col lg:flex-row gap-6">
          <div className="flex-1 min-w-0 space-y-6">
           <div className="rounded-3xl border border-white/5 bg-slate-900/50 p-5 sm:p-6 lg:p-8">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mt-4">
              ...
              <div><label className={labelCls}>Name of Repair Vendor</label><input disabled={isMasterLocked} value={req.repairVendorName || ""} onChange={(e) => updateReq({ repairVendorName: e.target.value })} className={inputCls} /></div>
              <div>
        ...
        <div className="w-full shrink-0 space-y-6 lg:w-80">
            <div className="space-y-3 rounded-3xl border border-white/5 bg-slate-900/50 p-5 sm:p-6">
