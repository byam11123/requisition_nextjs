            ...
            requests.map((row) => (
              <tr key={row.id} className={`hover:bg-white/[0.02] transition-colors cursor-pointer ${selectedIds.includes(String(row.id)) ? 'bg-indigo-500/5' : ''}`}>
                ...
                <td className="px-4 py-3 font-semibold text-indigo-400">
                  <Link href={`/dashboard/salary-advance/${row.id}`}>{row.requestId}</Link>
                </td>
                <td className="px-4 py-3">
                ...
                <td className="px-4 py-3 text-slate-300">{formatCurrency(row.totalAdvanceRequest)}</td>
                <td className="px-4 py-3 font-medium text-slate-100">{formatCurrency(row.balanceAdvance)}</td>
                <td className="px-4 py-3"><StatusChip tone={getStatusTone(row.status)}>{row.status}</StatusChip></td>
                <td className="px-4 py-3 text-right">
                  <ActionIconButton href={`/dashboard/salary-advance/${row.id}`} icon={Eye} label="View Details" tone="indigo" />
                </td>
              </tr>
            ))
          )}
