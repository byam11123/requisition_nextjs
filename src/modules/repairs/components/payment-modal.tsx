    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/70 backdrop-blur-sm">
      <div className="w-full max-w-lg rounded-3xl border border-[var(--app-border)] bg-[var(--app-surface-strong)] p-6 shadow-2xl animate-fade-in-up">
        <div className="mb-6 flex items-start justify-between">
          ...
          <button onClick={onClose} className="p-2 rounded-xl hover:bg-white/5"><X size={20} /></button>
        </div>
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div className="grid grid-cols-2 gap-3">
            <div><label className={labelCls}>UTR Number</label><input value={form.utrNo} onChange={e => setForm({...form, utrNo: e.target.value})} className={inputCls} required /></div>
            <div><label className={labelCls}>Mode</label>
              <FormSelect value={form.paymentMode} options={[{value:'UPI', label:'UPI'}, {value:'NEFT', label:'NEFT'}]} onChange={val => setForm({...form, paymentMode: val})} />
            </div>
          </div>
          <div className="grid grid-cols-2 gap-3">
            ...
            <div><label className={labelCls}>Amount</label><input type="number" value={form.amount} onChange={e => setForm({...form, amount: e.target.value})} className={inputCls} /></div>
          </div>
          <div><label className={labelCls}>Payment Proof</label><input type="file" onChange={e => setFile(e.target.files?.[0] || null)} className={inputCls} /></div>
          
          <div className="mt-6 flex gap-3">
            <button type="button" onClick={onClose} className="flex-1 py-2.5 rounded-xl border border-white/10">Cancel</button>
            <button type="submit" disabled={loading} className="flex-1 py-2.5 rounded-xl bg-emerald-600 font-medium">
